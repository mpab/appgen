...
def generate_single(ctx, entity):
    ...
    for table in ctx.TABLE_DEFINITIONS:
        if table == ctx.ENTITY_TABLE_NAME:
            continue  # don't add api entity as a constraint or a bare query
        ...
        if (
            ...
            continue  # don't add fields entity as a constraint
        else:  # get collection
    ...
    with open(f"{ctx.COMPONENTS_API_ENDPOINTS_PATH}/be4fe-paged.js", "r") as file:
        filedata = file.read()
    ...
    # save
    api_file = f"{ctx.API_APP_SRC_ENDPOINTS_PATH}/" + entity_names.KEBAB + ".js"
    ctx.save_data(api_file, filedata)

    # configure endpoint
    endpoint_cmd = f"expose(require('./endpoints/{entity_names.KEBAB}'));"
    endpoints_fp = f"{ctx.API_APP_SRC_PATH}/endpoints.js"
