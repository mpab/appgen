...
def _parse_schema_file(ctx, filename, depth):
    ...
    names = ctx.generate_entity_names_from_snake(entity_name)

    if depth == 0:
        ctx.set_entity_names_from_pascal(names.PASCAL)
        # TODO: add --url= option
        ctx.API_URL = (
            f"http://' + window.location.hostname + ':3000/api/{ctx.ENTITY_PASCAL}"
        )
