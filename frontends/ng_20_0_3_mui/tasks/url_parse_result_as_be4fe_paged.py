from ng_20_0_3_mui.tasks._parse_entity import parse_url_entity
...
def exec(ctx):
    ...
    for reference_entity in ctx.REFERENCES:
        reference_entity_id = ctx.generate_entity_names_from_snake(reference_entity).ID
        ctx.ENTITY_DEFINITIONS[reference_entity] = parse_url_entity(
            reference_entity, reference_entity_id, ctx.REFERENCES[reference_entity][0]
        )
        ctx.ENTITY_REF_MAP[reference_entity_id] = reference_entity
        if not reference_entity_id in ctx.ENTITY:
            ctx.fatal(
                f"expected: {ctx.ENTITY_SNAKE}.{reference_entity_id} from reference {reference_entity}, found: none"
            )
