...
def generate_single(ctx, entity):
    ...
    sql_file = f"{ctx.APP_DATABASE_SQL_PATH}/drop-table-" + entity_names.KEBAB + ".sql"
    ctx.save_data(sql_file, sql_drop_table)

    sql_file = (
        f"{ctx.APP_DATABASE_SQL_PATH}/create-table-" + entity_names.KEBAB + ".sql"
    )
    ...
    recreate_tables_fp = f"{ctx.APP_DATABASE_SCRIPTS_PATH}/recreate-tables"
    ...
    # seed table
    seed_file_fp = f"{ctx.APP_DATABASE_SCRIPTS_PATH}/seed-table-" + entity_names.KEBAB
    with open(f"{ctx.COMPONENTS_DB_PATH}/seed-table", "r") as file:
        filedata = file.read()
    # grep contents
    ...
    seed_all_tables_fp = f"{ctx.APP_DATABASE_SCRIPTS_PATH}/seed-tables"
