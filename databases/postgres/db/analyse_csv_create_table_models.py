...
def description(ctx):
    return f"analyse {ctx.ENTITY_ARG} (and dependencies), create table models using constraints"


def exec(ctx):
    analyze_csv(ctx)
    ctx.info(f'created table definitions for: {", ".join(ctx.TABLE_DEFINITIONS)}')
...
def analyze_csv(ctx):
    ...
    if ctx.has_opt(ctx.ENTITY_FIELDS_STEM):
        ...
        ctx.info(
            f"created entity fields table definition: {ctx.ENTITY_ARG}->{fields_dataframe.name}"
        )
        # TODO: check #columns
...
def analyze_table(ctx, df):
    ...
    for column, data_type, is_null in zip(df.columns, df.dtypes, df.isnull().any()):
        ...
        if column.endswith("_enum"):
            ...
            enum_fp = f"{ctx.APP_DATABASE_CSV_SEED_PATH}/{column}.csv"
...
def analyze_enum(ctx, df, column, entity_name):
    ...
    ctx.info(f"created enum table definition: {ctx.ENTITY_ARG}->{df.name}")
