...
class Context:
    ...
    def set_job(cls, job):
        ...
        cls.COMPONENTS_COMMON = f'{os.environ["__APPGEN_COMMON_PATH__"]}/components'
        cls.COMPONENTS_DB = f'{os.environ["__APPGEN_DB_PATH__"]}/components'
        cls.COMPONENTS_FE = f'{os.environ["__APPGEN_FE_PATH__"]}/components'
        cls.COMPONENTS_BE = f'{os.environ["__APPGEN_BE_PATH__"]}/components'
        ...
        cls.BACKEND_PATH = cls.match_opt_prefix_or_default(
            "--backend-path=", "./backend"
        )
        cls.ensure_folder(cls.BACKEND_PATH)

        cls.DATABASE_PATH = f"{cls.BACKEND_PATH}/database"
        cls.ensure_folder(cls.DATABASE_PATH)

        cls.DATABASE_SCRIPTS_PATH = f"{cls.DATABASE_PATH}/scripts"
        cls.ensure_folder(cls.DATABASE_SCRIPTS_PATH)

        cls.DATABASE_SQL_PATH = f"{cls.DATABASE_PATH}/sql"
        cls.ensure_folder(cls.DATABASE_SQL_PATH)

        cls.DATABASE_CSV_SEED_PATH = f"{cls.DATABASE_PATH}/csv_seed"
        cls.ensure_folder(cls.DATABASE_CSV_SEED_PATH)

        cls.API_APP_PATH = f"{cls.BACKEND_PATH}/api"
        cls.ensure_folder(cls.API_APP_PATH)

        cls.FRONTEND_PATH = cls.match_opt_prefix_or_default(
            "--frontend-path=", "./frontend"
        )
        cls.ensure_folder(cls.FRONTEND_PATH)
