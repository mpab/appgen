...
def exec(ctx):
    app_routes_fp = f'{ctx.APP_APP_PATH}/src/app/{TS_NAME}'
    ...
    if (f'{ctx.ENTITY_SNAKE_UCASE}_ROUTES' in filedata):
        ...
        route = '    , {' + f" path: '{ctx.ENTITY_PASCAL_SPACED_NO_ENUM}', loadChildren: () => import('../pages/{ctx.ENTITY_KEBAB}/{ctx.ENTITY_KEBAB}.routes').then(m => m.{ctx.ENTITY_SNAKE_UCASE}_ROUTES)" + ' }'
