...
def exec(ctx):
    ...
    for reference_entity in ctx.REFERENCES:
        ...
        entity_references_imports.append("import { " + names.PASCAL + "Model }" + f" from '../{names.KEBAB}/{names.KEBAB}.model';")
    ...
    for field_idx, field in enumerate(entity_fields):
        ...
        if field_name in ctx.ENTITY_REF_MAP.keys():
            ...
            names = ctx.from_snake(reference_entity)
            entity_references_declarations.append(f'{ctx.TAB08}{reference_entity}: {names.PASCAL}Model[],')
        
    with open(f'{ctx.PAGE_BE4FE_PAGED_TEMPLATE_PATH}/_{TS_NAME}', 'r') as file:
        filedata = file.read()
