...
def generate_single(ctx, entity):
    ...
    sql_get_index = f'"select pg_get_serial_sequence(' + f"'{entity}', '{entity_names.ID}'" + ')"' # result assigned to 'index_name'
    sql_reset_index = '`select setval(' + "'${index_name}'" + f',(select max({entity_names.ID}) from {entity}))`'

    sql_read = f'`select * from {entity} order by "' + entity_names.ID + '";`'
    sql_read_enum_as_array = f'`select array(select {entity} from {entity} order by "' + entity_names.ID + '");`'
    sql_read_item = f'`select * from {entity} where "' + \
        entity_names.ID + '" = ' + '${' + entity_names.ID + '};`'
    sql_delete_item = f'`delete from {entity} where "' + \
        entity_names.ID + '" = ' + '${' + entity_names.ID + '};`'
    ...
    kv_assign = list(map(lambda k, v: k + '=' + v,
                     sql_items_k_quoted, items_v))
    kv_assign_csv = ','.join(kv_assign)

    sql_create_item = '`insert into ' + entity + \
        ' (' + sql_items_k_quoted_csv + ') values (' + \
        sql_items_v_csv + ') returning *;`'
    sql_update_item = '`update ' + entity + ' set ' + kv_assign_csv + \
        ' where "' + entity_names.ID + '" = ${' + entity_names.ID + '} returning *;`'
